/**
 * Hub CSV - Portal Authentication System v1.0.0
 * Protege portais de clientes com autenticação por senha.
 * Inclui blur overlay, modal de login, gestão de sessão.
 *
...
(function () {
  ...
  async function verifyToken(token) {
    try {
      const resp = await fetch(AUTH_API + '/verify', {
  ...
  async function doLogin(email, password) {
    const resp = await fetch(AUTH_API + '/login', {
      ...
      body: JSON.stringify({ portal: PORTAL, email, password }),
    });
    const data = await resp.json();
  ...
  const colors = PORTAL_COLORS[PORTAL] || PORTAL_COLORS.unimed;
  ...
  function showAuthOverlay() {
    ...
    const existing = document.getElementById('hub-auth-overlay');
        ...
        <div class="hub-auth-body">
          ...
          <form id="hub-auth-form" autocomplete="on">
            ...
            <div class="hub-auth-field">
              ...
              <input type="password" id="hub-auth-password" name="password" placeholder="Digite a senha do portal" required autocomplete="current-password">
            </div>
    ...
    // Form handler
    const form = document.getElementById('hub-auth-form');
    const errorEl = document.getElementById('hub-auth-error');
    const submitBtn = document.getElementById('hub-auth-submit');
    const btnText = document.getElementById('hub-auth-btn-text');
    const spinner = document.getElementById('hub-auth-spinner');
  ...
  function addLogoutButton() {
    ...
    document.getElementById('hub-auth-logout-btn').addEventListener('click', function (e) {
      e.preventDefault();
      doLogout();
      const logoutEl = document.getElementById('hub-auth-logout');
  ...
  async function init() {
    ...
    if (session && session.token) {
      // Verificar token no servidor (em background)
      const valid = await verifyToken(session.token);
      if (valid) {
        addLogoutButton();
        return; // Sessão válida, não mostrar overlay
      }
      clearSession();
    }
